        ...
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <div className="glass-card lg:col-span-2">
              <h2 className="text-lg font-semibold mb-4 text-primary-light">File Type Hierarchy</h2>
              {tree?.folders ? (
                <FileTypeTreemap
            ...
            <div className="glass-card space-y-6">
              ...
              <div>
                ...
                {filterLoading ? (
                  ...
                  <div className="space-y-1.5">
                    {filteredTopFiles.slice(0, 8).map((f, i) => (
                      <div key={i} className="flex items-center justify-between text-sm bg-surface-lighter rounded-lg px-3 py-2">
                        <span className="truncate text-text-primary">{f.path.split(/[\\/]/).pop()}</span>
                        <span className="text-text-secondary text-xs shrink-0 ml-2">{formatBytes(f.size)}</span>
                      </div>
                    ))}
                  </div>
                ) : (
              ...
              {!filterLoading && filteredColdFiles.length > 0 && (
                <div>
                  ...
                  <div className="space-y-1.5">
                    {filteredColdFiles.slice(0, 5).map((f, i) => (
                      <div key={i} className="flex items-center justify-between text-sm bg-surface-lighter rounded-lg px-3 py-2">
                        <span className="truncate text-text-primary">{f.path.split(/[\\/]/).pop()}</span>
                        {f.size !== undefined ? (
                          <span className="text-text-secondary text-xs shrink-0 ml-2">{formatBytes(f.size)}</span>
                        ) : f.usage_count !== undefined ? (
                          <span className="text-text-secondary text-xs shrink-0 ml-2">Usage: {f.usage_count}</span>
                        ) : null}
                      </div>
                    ))}
                  </div>
